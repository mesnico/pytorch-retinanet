...
def main(args=None):
	...
	parser.add_argument('--checkpoint_interval', help='Iterations before saving an intermediate checkpoint', type=int, default=5000)
	...
	if parser.dataset == 'coco':
		...
		dataset_train = CocoDataset(parser.data_path, set_name='train2017', transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
		...
		dataset_train = OidDataset(parser.data_path, subset='train',
								transform=transforms.Compose([Normalizer(), Augmenter(), Resizer(min_side=400, max_side=600)]))
	...
	experiment_fld = 'experiment_{}_resnet{}_{}'.format(parser.dataset, parser.depth, time.strftime("%Y%m%d%H%M%S", time.localtime()))
